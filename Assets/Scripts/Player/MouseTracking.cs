    ...
    private void TrackMouseMovement()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            if (Input.mousePosition.y > mouseDownPos.y && swipeDistanceY > trackSensitivity)
            {
                ...
                {
                    mouseTrackingObject.transform.rotation = Quaternion.Euler(0, 0, -45); // Rotate the object to the top right
                }
                else if (Input.mousePosition.x < mouseDownPos.x && swipeDistanceX < trackSensitivity)
                {
                    mouseTrackingObject.transform.rotation = Quaternion.Euler(0, 0, 45); // Rotate the object to the top left
                }
                else
                {
                    mouseTrackingObject.transform.rotation = Quaternion.Euler(0, 0, 0); // Rotate the object to the top
                }
            }
            else if (Input.mousePosition.y < mouseDownPos.y && swipeDistanceY < trackSensitivity)
            {
                if (Input.mousePosition.x > mouseDownPos.x && swipeDistanceX > trackSensitivity)
                {
                    mouseTrackingObject.transform.rotation = Quaternion.Euler(0, 0, -135); // Rotate the object to the bottom right
                }
                else if (Input.mousePosition.x < mouseDownPos.x && swipeDistanceX < trackSensitivity)
                {
                    mouseTrackingObject.transform.rotation = Quaternion.Euler(0, 0, 135); // Rotate the object to the bottom left
                }
                else
                {
                    mouseTrackingObject.transform.rotation = Quaternion.Euler(0, 0, 180); // Rotate the object to the bottom
                }
            }
            else
            {
                if (Input.mousePosition.x > mouseDownPos.x && swipeDistanceX > trackSensitivity)
                {
                    mouseTrackingObject.transform.rotation = Quaternion.Euler(0, 0, -90); // Rotate the object to the right
                }
                else if (Input.mousePosition.x < mouseDownPos.x && swipeDistanceX < trackSensitivity)
                {
                    mouseTrackingObject.transform.rotation = Quaternion.Euler(0, 0, 90); // Rotate the object to the left
                }
            }
